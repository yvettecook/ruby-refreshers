...
describe 'the Friday test :)' do
  ...
  fspecify 'all_elements_except_first_3' do
    n = all_elements_except_first_3 [1, 2, 3, 4, 5, 6, 7]
    expect(n).to eq [4, 5, 6, 7]
  end

  fspecify 'add_element_to_beginning_of_array' do
  ...
  fspecify 'number_of_elements_that_are_palindromes' do
  ...
  fspecify 'longest_word_in_array' do
  ...
  fspecify 'double_array' do
    n = double_array [1, 2, 3]
    expect(n).to eq [1, 2, 3, 1, 2, 3]
  end

  fspecify 'turn_symbol_into_string' do
    n = turn_symbol_into_string :foobar
    expect(n).to eq 'foobar'
  end

  fspecify 'average_of_array' do
    n = average_of_array [10, 15, 25]
    expect(n).to eq 17
  end

  fspecify 'get_elements_until_greater_than_five' do
  ...
  fspecify 'round_down_number' do
    n = round_down_number 4.9
    expect(n).to eq 4
  end

  fspecify 'format_date_nicely' do
